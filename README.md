...
### 从 v1.1.0 在线更新到 v1.2.7

保留现有 `.env` 中的 `POSTGRES_PASSWORD`、`DATABASE_URL`、两条 JWT 密钥和全部数据路径，只把三条应用镜像改为：

```bash
PROXY_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-proxy:v1.2.7
WEB_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-web:v1.2.7
API_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-api:v1.2.7
```
...
v1.2.7 兼容 NAS/隧道终止 HTTPS 的远程访问：同一公开 Host 的 HTTP/HTTPS 来源都会被接受，避免手机端登录提示“请求来源不受信任”。
