...
- 首页日历支持日/三日/周/月视图；时间轴可拖动个人日程调整日期、时间和时长，日程备注直接显示在日历中，并默认显示可关闭的中国法定节假日、调休、二十四节气及常用传统节日。
- 轻量个人待办保存在数据库中，不要求日期或额外字段，可单行快速添加、勾选完成和直接删除。
...
v1.24.0 将个人待办收紧为无日期的勾选清单：顶部单行快速添加，每项可直接完成、恢复或删除；日程备注继续直接显示在日历中。

v1.24.1 将上传目录初始化命令完整放入容器入口参数，避免极空间将 Compose 的字符串 `command` 拆分后出现 `mkdir: missing operand`。

v1.23.0 在首页日历加入私有个人待办数据与入口；同时把日程备注直接呈现在月、周、日和当天安排中。

v1.22.0 为不同手帐作者分配稳定的低饱和明亮配色；时光流卡片、翻页视图和正文内评论会沿用作者颜色，方便快速分辨两个人留下的内容。
...
v1.9.0 将手帐时光流升级为多卡片纵向浏览：当前篇与前后多篇正文摘要同时显示，拖动右侧历史轴时卡片连续滑动并在上下边缘渐隐；时光流不再加载或统计评论，附近正文通过单次小批量接口按需读取。

v1.8.1 优化手帐与日历同步体验：手帐保存改为局部索引更新并缓存最近正文和评论；日程保存、删除、拖动和调整时长均直接更新当前页面，不再重新加载手帐、任务和整份日历。
