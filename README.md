...
### 从 v1.1.0 在线更新到 v1.2.3

保留现有 `.env` 中的 `POSTGRES_PASSWORD`、`DATABASE_URL`、两条 JWT 密钥和全部数据路径，只把三条应用镜像改为：

```bash
PROXY_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-proxy:v1.2.3
WEB_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-web:v1.2.3
API_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-api:v1.2.3
```
