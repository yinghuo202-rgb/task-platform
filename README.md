...
### 从 v1.1.0 在线更新到 v1.2.8

保留现有 `.env` 中的 `POSTGRES_PASSWORD`、`DATABASE_URL`、两条 JWT 密钥和全部数据路径，只把三条应用镜像改为：

```bash
PROXY_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-proxy:v1.2.8
WEB_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-web:v1.2.8
API_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-api:v1.2.8
```
...
v1.2.8 修复极空间离线 Compose 未把 `ALLOWED_ORIGINS` 传入 API 的问题。若通过极空间本机转发访问，默认允许 `http://127.0.0.1:8081`。
