...
### 从 v1.1.0 在线更新到 v1.2.4

保留现有 `.env` 中的 `POSTGRES_PASSWORD`、`DATABASE_URL`、两条 JWT 密钥和全部数据路径，只把三条应用镜像改为：

```bash
PROXY_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-proxy:v1.2.4
WEB_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-web:v1.2.4
API_IMAGE=ghcr.io/yinghuo202-rgb/task-platform-api:v1.2.4
```
...
v1.2.4 会在 API 启动前自动初始化 `/data/uploads/journal` 并修正绑定目录权限，避免极空间等 NAS 使用 root-owned 数据目录时出现 `EACCES: permission denied, mkdir '/data/uploads/journal'`。
